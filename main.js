...
function Gost(encryptKey) {
    ...
    if (encryptKey.length != 8){
        console.log("Длина ключа должна быть 8 символов");
    }
    ...
    this.crypt = function (text, mode) { //входной текст + encrypt/decrypt
        var xorResult = '';
        var result = '';
...
function getBinary(string) { // строка в двоичную систему
  let binary = "";
...
function getText(binary) { //двоичную систему в строку
  let string = "";
...
//  console.log(longKey);
    ...
    console.log(chToByte("gsklergjnawlgj"));
    var M = chToByte(plaintext);
    var H = key;
    let encr = "";
    var hash = 0;
    console.log(M);
  ...
  for (let i = 0; i < binary1.length; i++) {
    var EncryptedMessage = new Gost(key);
    encr = String(encr.concat(EncryptedMessage.crypt(String(M[i]),'encrypt')));
    H = xor(getBinary(encr), getBinary(H));
    hash = H;
  }


  document.getElementById("output").innerHTML = getText(hash);
}